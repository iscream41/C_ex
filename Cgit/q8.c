...
void showResult(FILE* fp, pro* result, int index, int numl, int namel, int pricel, int i);
...
int main() {
    ...
    //read csv
    if( (fp = fopen("test.csv","r,css=UTF-8")) == NULL ) {
        printf("not");
        exit(1);
    }
    ...
    //出力処理
    //--------------------------------------------
    showResult(fp, sw, indexL, numL, nameL, priceL,i);
...
void showResult(FILE* fp, pro* result, int index, int numl, int namel, int pricel,int i)
{
    ...
    for(loop = 0; loop < i + 1; loop++) {
        ...
        if(loop == 0) {
            ...
            fprintf(fp, "%s,%s,%s\n",result->num,result->name,result->price);
