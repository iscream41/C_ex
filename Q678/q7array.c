...
int main() {
   ...
   char inList[4][20] = {{"insert"},{"00007"},{"aaaaaaaaaa"},{"10000000"}};
   char inList2[4][20] = {{"insert"},{"00008"},{"bbbbbbbb"},{"10000000"}};
...
pro* insert(pro* adhead, pro* head, char insertList[][20]) {
    ...
    //headが先頭のポインタ
     ...
     //追加する構造体の生成
     strcpy((head + iIndexCount)->num,insertList[1]);
     strcpy((head + iIndexCount)->name,insertList[2]);
     strcpy((head + iIndexCount)->price,insertList[3]);

     

    (head + iIndexCount)->num_l = GetCharByte((head + iIndexCount)->num);
    numL = Maxlength(numL, (head + iIndexCount)->num_l);

    (head + iIndexCount)->name_l = GetCharByte((head + iIndexCount)->name);
    nameL = Maxlength(nameL, (head + iIndexCount)->name_l);

    (head + iIndexCount)->price_l = GetCharByte((head + iIndexCount)->price);
    priceL = Maxlength(priceL, (head + iIndexCount)->price_l);
    
    
    //先頭に追加する場合
    if(atoi(adhead->num) > atoi((head + iIndexCount)->num) ){
        
        //新しい構造体が先頭に
        (head + iIndexCount)->prev = NULL;
        (head + iIndexCount)->next = adhead;
        
        //先頭だった構造体を2番目に
        adhead->prev = (head + iIndexCount);
        
        
        //showは先頭のnextから表示
        return (head + iIndexCount);
    ...
    //末尾に追加
     ...
     while(1) {
         ...
         if(atoi(adhead->num) > atoi( (head + iIndexCount)->num) ) {
            //n-1の構造体
             adhead->prev->next = (head + iIndexCount);
             
             //追加する構造体
             (head + iIndexCount)->prev = adhead->prev;
             (head + iIndexCount)->next = adhead->next;
         ...
         //末尾に挿入
             ...
             adhead->next = (head + iIndexCount);
             
             (head + iIndexCount)->prev = adhead;
             (head + iIndexCount)->next = NULL;
             
             break;
         } 
         //上書きはエラー
         else if(adhead->num == (head + iIndexCount)->num) {
             printf("すでに存在。\n");
             break;
         }
